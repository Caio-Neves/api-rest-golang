...
func InitAdminCategoriesRoutes(m *mux.Router, h handlers.CategoryHandler) {
	...
	r.HandleFunc("",
		middlewares.ValidateSupportedMediaTypes([]string{"application/json"}, h.CreateCategory)).Methods(http.MethodOptions,
		http.MethodPost)
	r.HandleFunc("/_delete",
		...
		http.MethodPost)
	r.HandleFunc("/{id}",
		middlewares.ValidateSupportedMediaTypes([]string{"application/json"}, h.UpdateCategoryFields)).Methods(http.MethodOptions,
		http.MethodPatch)
